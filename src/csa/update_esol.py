...
def update_bank(current_bank, child, dcut):
    ...
    for entry in all_population:
        esol = entry['scores'].get('esol', None)
        if esol is None:
            raise ValueError("Error: ESOL score is missing or None. Make sure ESOL was computed for all entries.")
        esols.append(esol)

    min_esol = min(esols)
    max_esol = max(esols)
    denom_esol = max_esol - min_esol

    if denom_esol < 1e-9:
        raise ValueError(
            f"Error: Max and Min ESOL are identical ({min_esol}). "
        )

    # -----------------------------
    # 3) Store norms + objective
    # -----------------------------
    for entry in all_population:
        aff = entry['scores']['affinity']
        esol = entry['scores']['esol']

        # affinity_norm (higher is better)
        neg_aff = -aff
        norm_aff = (neg_aff - min_aff) / denom_aff
        entry['scores']['affinity_norm'] = norm_aff

        # esol_norm (higher is better)
        norm_esol = (esol - min_esol) / denom_esol
        entry['scores']['esol_norm'] = norm_esol

        # Objective: 1*norm_esol + 0.3*norm_aff
        entry['scores']['objective'] = norm_esol + norm_aff
